...
// The application initialization and lifecycle is now managed by ApplicationController
// This simplified main.ts just creates the controller and lets it handle everything
// All the OpenAI initialization, service management, and IPC handling is now properly separated
    }
...
3. Get intelligent, context-aware debugging assistance!`;
    ...
    // Settings window is now handled by WindowManager service
    ...
    private setupIpcHandlers(): void {
        ...
        ipcMain.handle('global-rag-add-folder', async (event, folderPath: string) => {
            ...
            try {
                const result = await this.globalRagService.indexFolder(folderPath);
        ...
        ipcMain.handle('global-rag-refresh', async () => {
            ...
            try {
                const result = await this.globalRagService.refreshGlobalKnowledgeBase();
        ...
        ipcMain.handle('global-rag-clear', async () => {
            ...
            try {
                await this.globalRagService.clearGlobalKnowledgeBase();
        ...
        ipcMain.handle('global-rag-get-status', async () => {
            ...
            try {
                const stats = await this.globalRagService.getStats();
                const status = {
                    totalDocuments: stats.totalDocuments,
                    folders: [], // TODO: Implement folder tracking
                    lastUpdated: stats.lastUpdate,
                    isReady: this.globalRagService.isReady(),
        ...
        // Note: Individual folder removal not implemented in current GlobalRAGService architecture
        // The global knowledge base treats all documents as a single collection
        // Use 'global-rag-clear' to clear all documents

        console.log('🔧 [IPC] IPC handlers setup complete (including Global RAG handlers)');
    }
}
